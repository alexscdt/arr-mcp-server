...
export const addMovieToolDefinition: Tool = {
    name: 'add_movie',
    description:
        'Add a movie to Radarr and trigger a search for a download. Requires a tmdbId (get it from search_movie first). By default uses the first available quality profile and root folder.',
    inputSchema: {
        type: 'object',
        properties: {
            tmdbId: {
            ...
            qualityProfileName: {
                type: 'string',
                description:
                    'Optional quality profile name (e.g. "HD-1080p", "Ultra-HD", "Any"). Falls back to the first profile if not specified.'
            },
            searchNow: {
                type: 'boolean',
...
export async function handleAddMovie(args: unknown): Promise<string> {
    const input: AddMovieInput = addMovieInputSchema.parse(args)
