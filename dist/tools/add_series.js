...
export const addSeriesToolDefinition = {
    name: 'add_series',
    description: 'Add a TV series to Sonarr and trigger a search for missing episodes. Requires a tvdbId (get it from search_series first). By default monitors all seasons.',
    inputSchema: {
        type: 'object',
        properties: {
            tvdbId: {
            ...
            qualityProfileName: {
                type: 'string',
                description: 'Optional quality profile name (e.g. "HD-1080p", "Any"). Falls back to the first profile if not specified.'
            },
            monitor: {
                type: 'string',
                enum: ['all', 'future', 'missing', 'existing', 'firstSeason', 'latestSeason', 'none'],
                description: 'What to monitor: all seasons (default), only future episodes, only missing, etc.'
            },
            searchNow: {
                type: 'boolean',
...
export async function handleAddSeries(args) {
    const input = addSeriesInputSchema.parse(args);
...
//# sourceMappingURL=add_series.js.map