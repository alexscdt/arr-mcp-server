...
export const getLibraryToolDefinition = {
    name: 'get_library',
    description: 'Get an overview of the Plex library. Can list all libraries, recently added items, or search for a specific title within Plex.',
    inputSchema: {
        type: 'object',
        properties: {
            mode: {
                ...
                description: 'What to fetch: "overview" (list of libraries with counts), "recent" (recently added items), "search" (find a title in Plex).'
            },
            query: {
                type: 'string',
...
async function handleSearch(query, limit) {
    ...
    const filtered = items.filter((item) => ['movie', 'show', 'season', 'episode'].includes(item.type));
...
export async function handleGetLibrary(args) {
...
//# sourceMappingURL=get_library.js.map